    ...
    try:
        print_std_out_colored(
            *{
                ...
                TestResult.STACK_OVERFLOW: (ConsoleColor.FG_RED, "STACK OVERFLOW"),
            }[test_result]
        )
    except KeyError:
...
def print_test_info(test_result, test_nr, total_tests, diagnostic, timer):
    ...
    caret_at_line_start = print_test_info.__dict__.get("caret_at_line_start", True)
    ...
    print(
        " ({:>{test_nr_w}}/{})".format(
            test_nr, total_tests_str, test_nr_w=len(total_tests_str)
        ),
        end="",
        flush=True,
    )
...
def print_failed_test(param_names, arguments, test_failure, res_printer):
    ...
    if properties:
        ...
        for prop in properties:
            ...
            print(": {}{}".format(gen_spaces(max_col_size - len(prop.name())), value))
